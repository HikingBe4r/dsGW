...
<body>
	...
	<div class="row">
		...
		<!-- 사원으로 수정,삭제 지우기 -->
		<c:if
			test="${sessionScope.employee.id eq requestScope.article.employeeId}">
	...
	<div class="row">
		<div class="col-md-11">
			<textarea rows="2" cols="180" id="content"></textarea>
		</div>
	...
	<div class="row" id="replyList">
		<c:forEach var="replys" items="${requestScope.article.replys}">
			<div>
				<div class="col-md-10">
					<textarea rows="2" cols="150" readonly>${pageScope.replys.content }</textarea>
				</div>
				<br> <label>${pageScope.replys.employee.name}</label><label>${pageScope.replys.writeday}</label>
				<div class="col-md-1">
					<button id="${pageScope.replys.id}">수정</button>
				</div>
				<div class="col-md-1">
					<button id="${pageScope.replys.id}">삭제</button>
				</div>
			</div>
		</c:forEach>
	</div>
